                    ...
                    <div class="bg-[#fff8f0] border border-[#8d001f]/30 rounded-3xl p-7 shadow-lg">
                        <div class="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
                            <div>
                                ...
                                <p class="text-[#1a0b0f]/80 text-sm mt-1">
                                    Status: 
                                    @if($turnamen->status_pembayaran == 'Menunggu')
                                        <span class="bg-yellow-500/20 text-yellow-700 px-3 py-1 rounded-full font-bold">Menunggu Konfirmasi</span>
                                    @elseif($turnamen->status_pembayaran == 'Lunas')
                                        <span class="bg-green-500/20 text-green-700 px-3 py-1 rounded-full font-bold">Terdaftar</span>
                                    @elseif($turnamen->status_pembayaran == 'Ditolak')
                                        <span class="bg-red-500/20 text-red-700 px-3 py-1 rounded-full font-bold">Ditolak</span>
                                    @else
                                        <span class="bg-gray-500/20 text-gray-700 px-3 py-1 rounded-full font-bold">Belum Bayar</span>
                                    @endif
                                </p>
                            </div>
                            ...
                            <div class="flex flex-col gap-2 mt-4 md:mt-0">
                                @if($turnamen->status_pembayaran != 'Lunas')
                                    <form action="{{ route('siswa.turnamen.upload', $turnamen->id) }}" method="POST" enctype="multipart/form-data">
                                        @csrf
                                        <input type="file" name="bukti_pembayaran" class="text-sm mb-2 text-black" required>
                                        <button type="submit" class="bg-[#8d001f] text-[#fff8e7] px-5 py-2 rounded-xl font-bold hover:bg-[#d4af37] transition">
                                            Upload Bukti
                                        </button>
                                    </form>
                                @else
                                    <a href="{{ asset('uploads/bukti_turnamen/'.$turnamen->bukti_pembayaran) }}" target="_blank"
                                       class="bg-[#fff8f0] border border-[#d4af37] px-5 py-2 rounded-xl font-bold hover:bg-[#d4af37] hover:text-[#fff8e7] transition text-center">
                                        Lihat Bukti
                                    </a>
                                @endif
                            </div>
                    ...
                    <table class="w-full border border-[#8d001f]/20">
                        <thead class="bg-[#fff3e8]">
                            <tr>
                                ...
                                <th class="border p-3 text-left text-[#1a0b0f]/75">Tanggal</th>
                                <th class="border p-3 text-left text-[#1a0b0f]/75">Biaya</th>
                                <th class="border p-3 text-left text-[#1a0b0f]/75">Status</th>
                        ...
                        <tbody>
                            @forelse($riwayat as $item)
                                <tr class="hover:bg-[#fff3e8]/10 transition">
                                    <td class="border p-3 text-[#1a0b0f]">{{ $item->nama_turnamen }}</td>
                                    <td class="border p-3 text-[#1a0b0f]">
                                        {{ \Carbon\Carbon::parse($item->tanggal)->format('d F Y') }}
                                    </td>
                                    <td class="border p-3 text-[#1a0b0f]">Rp{{ number_format($item->biaya,0,',','.') }}</td>
                                    <td class="border p-3 text-[#1a0b0f]">
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="border p-3 text-center text-[#1a0b0f]/60">
